#!/usr/bin/env python3
"""Lossless real-time shipper (Windows) for EMS JSONL.

- Tails ems_telemetry.jsonl
...
Protocol:
- Client sends one JSON per line: {seq, ts, kind, payload}
- Server replies: {"ack": <seq>} one per line

Usage (Windows):
  python ems_shipper.py --file "C:\\Users\\xfive\\Desktop\\ems_sim\\ems_telemetry.jsonl" --host 127.0.0.1 --port 9010
...
def main():
    ...
    ap.add_argument("--file", required=True, help="ems_telemetry.jsonl")
    ap.add_argument("--host", default="127.0.0.1")
    ap.add_argument("--port", type=int, default=9010)
    ap.add_argument("--db", default=os.path.join(os.path.dirname(__file__), "ems_spool.db"))
    ...
    print(f"[shipper] tail={args.file}")
    print(f"[shipper] spool={args.db}")
    print(f"[shipper] target={args.host}:{args.port} (tunnel)")
    ...
    def ensure_conn():
        ...
        while sock is None:
            try:
                ...
                print("[shipper] connected")
            except Exception as e:
                print(f"[shipper] connect failed: {e}; retrying...")
                time.sleep(1.0)

    # Main loop: tail + enqueue, and continuously flush
    tail = tail_lines(args.file)
    last_flush = 0.0

    while True:
        # 1) consume a line if available
        line = next(tail)
        if not line:
            continue
        ...
        # send in seq order all unsent rows
        ...
        try:
            ...
            print(f"[shipper] send/ack failed: {e}; reconnecting")
            try:
                sock.close()
            except Exception:
                pass
